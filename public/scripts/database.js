async function showPartDescription(id) { //показывает описание детали
    $.get("http://localhost:4015/components", function (data) {
...
async function getAnnotations(id) { //берёт аннотации к анимации процедуры из БД
    $.get("http://localhost:4015/procedures", function (data) {
...
async function showRequirements() { //показывает технические требования
    $.get("http://localhost:4015/other", function (data) {
        $("#info").html(data[0].description);
    });
    if(!isModelLoaded) loadModel();
}
...
async function showComments(id) {
    $.get("http://localhost:4015/comments", async function (data) {
...
async function showPartsAndTools() { //показывает инструменты и расходники
    let info = ``;
    $.get("http://localhost:4015/parts", function (data) {
        ...
        $.get("http://localhost:4015/tools", function (data) {
...
async function showDocuments() { //показывает документы
    $.get("http://localhost:4015/documents", function (data) {
...
async function getCurrentUser() {
    let result;
    $.ajax({
        url: "http://localhost:4015/currentuser",
        type: 'get',
        dataType: 'json',
        async: false,
...
async function logout() {
    $.get("http://localhost:4015/logout", function (data) {
